    ...
    function send_notification_emails($notification_id, $email_notify_to, $extra_data = array()) {
        ...
        if ($notification->category == "ticket" && $notification->event !== "ticket_assigned") {
            ...
            if ($notification->event == "contract_rejected") {
                $email_template = $ci->Email_templates_model->get_final_template("contract_rejected");
            } else if ($notification->event == "contract_accepted") {
                $email_template = $ci->Email_templates_model->get_final_template("contract_accepted");
            }
            ...
            if ($email_notify_to) {
                ...
                try {
                    ...
                    if ($email_notification_info_of_hook && is_array($email_notification_info_of_hook)) {
                        if (get_array_value($email_notification_info_of_hook, "subject")) {
                            $subject = get_array_value($email_notification_info_of_hook, "subject");
                        }
                        if (get_array_value($email_notification_info_of_hook, "message")) {
                            $message = get_array_value($email_notification_info_of_hook, "message");
                        }
                        if (get_array_value($email_notification_info_of_hook, "email_options")) {
                            $email_options = get_array_value($email_notification_info_of_hook, "email_options");
                        }
                        if (get_array_value($email_notification_info_of_hook, "attachement_url")) {
                            $attachement_url = get_array_value($email_notification_info_of_hook, "attachement_url");
                        }
                    }
                } catch (\Exception $ex) {
                    log_message('error', '[ERROR] {exception}', ['exception' => $ex]);
